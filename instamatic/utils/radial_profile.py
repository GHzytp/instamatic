...
from .local_max import local_max, points_todim
from .math import sum_functions, lkp_funcs
from .distortion import rad_dis, filter_ring, optimize_center, optimize_distortion, points_topolar
from .vizualize import plot_fit, plot_points, plot_distpolar
...
def calc_polarcoords(center, dims, ns=None, dists=None):
    ...
    if ns is not None:
        for i in range(len(ns)):
            rs /= rad_dis(thes, dists[i * 2 + 1], dists[i * 2 + 2], ns[i])
...
def residuals_fit(param, r, intens, funcs):
    ...
    return intens - sum_functions(r, funcs, param)
...
def fit_radialprofile(r, intens, funcs, init_guess, maxfev=None):
    ...
    try:
        ...
        for i in range(len(funcs)):
            assert (funcs[i] in lkp_funcs)

        init_guess = np.array(init_guess)
        init_guess = np.reshape(init_guess, sum(map(lambda x: lkp_funcs[x][1], funcs)))
...
def run_singleImage(img, dims, settings, show=False):
    ...
    points = local_max(img, mysettings['lmax_r'], mysettings['lmax_thresh'])
    points = points_todim(points, dims)

    # convert center to real space
    center_init = points_todim(mysettings['lmax_cinit'], dims)

    # filter to single ring
    points = filter_ring(points, center_init, mysettings['lmax_range'])

    if show:
        if settings['plt_imgminmax'] is None:
            mysettings['plt_imgminmax'] = (0., 1.)

        plot = plot_points(img, points, vminmax=mysettings['plt_imgminmax'], dims=dims, invert=True,
                                      show=show)

    # optimize center
    center = optimize_center(points, center_init, verbose=show)

    # fit distortions
    points_plr = points_topolar(points, center)
    dists = optimize_distortion(points_plr, mysettings['ns'])
    if show:
        plot = plot_distpolar(points_plr, dims, dists, mysettings['ns'], show=show)

    # calc coordinates in optimized system
    rs, thes = calc_polarcoords(center, dims, mysettings['ns'], dists)
    ...
    R, I = calc_radialprofile(img, rs, mysettings['rad_rmax'], mysettings['rad_dr'],
    ...
    res_back = fit_radialprofile(fit_R, fit_I, funcs_back, mysettings['back_init'],
                                                            maxfev=1000)
    if show:
        plot = plot_fit(R, I, dims, funcs_back, res_back, show=show)

    I = I - sum_functions(R, funcs_back, res_back)

    # fit
    res = fit_radialprofile(R, I, mysettings['fit_funcs'], mysettings['fit_init'],
                                                       maxfev=mysettings['fit_maxfev'])

    if show:
        plot = plot_fit(R, I, dims, mysettings['fit_funcs'], res, show=show)
