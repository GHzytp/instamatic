...
from .math import lkp_funcs, sum_functions
from .distortion import rad_dis
...
def plot_fit(r, intens, dims, funcs, param, show=False):
    ...
    try:
        ...
        param = np.reshape(param, sum(map(lambda x: lkp_funcs[x][1], funcs)))
    ...
    n = 0
    for i in range(len(funcs)):
        ax.plot(r, lkp_funcs[funcs[i]][0](r, param[n:n + lkp_funcs[funcs[i]][1]]),
                'g-')
        n += lkp_funcs[funcs[i]][1]
    # sum of functions
    sum_funcs = sum_functions(r, funcs, param)
